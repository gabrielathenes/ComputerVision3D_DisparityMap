                    ...
                    const Image<byte>& im2, int i2,int j2,float m2)
{
    ...
    for (int i=-win;i<win+1;i++){
        for (int j=-win;j<win+1;j++){
            ...
            dist_left+=pow(im1(i1+i,j1+j)-m1,2);
            dist_right+=pow(im2(i2+i,j2+j)-m2,2);
        }
    }
    dist=dist/(sqrt(dist_left*dist_right));
    return dist;
}
...
static float sum(const Image<byte>& im, int i, int j)
{
    float s=0.0f;
    // ------------- TODO -------------
    for (int k=-win;k<win+1;k++){
        for (int l=-win;l<win+1;l++){
            s+=im(i+k,j+l);
        }
    }
    return s;
...
static void find_seeds(Image<byte> im1, Image<byte> im2,
    ...
    for(int y=win; y+win<maxy; y++) {
        ...
        for(int x=win; x+win<im1.width(); x++) {
            ...
            float NCC = 0.0f;
            int disparity = 0 ;
            for(int i=dMin;i<dMax+1;i++){
                if ((win<=x+i) && (ccorrel(im1,x,y,im2,x+i,y)>NCC)) {
                    NCC=ccorrel(im1,x,y,im2,x+i,y);
                    disparity = i ;
                }
            }
        ...
        for(int i=0; i<4; i++) {
            ...
            if(0<=x-win && 0<=y-win &&
               ...
               ! seeds(x,y)) {
                ...
                float score = -1.0f;
                for (int k=-1; k<2; k++){
                    if ((0<=x+disp(s.x,s.y)+k-win)&&(dMin<=disp(s.x,s.y)+k)&&(disp(s.x,s.y)+k<=dMax)&&(ccorrel(im1,x,y,im2,x+disp(s.x,s.y)+k,y)>score)){
                        score=ccorrel(im1,x,y,im2,x+disp(s.x,s.y)+k,y);
                        disp(x,y)=disp(s.x,s.y)+k;
                    }
                }
                Seed neighbor = Seed(x,y,disp(x,y),score);
                Q.push(neighbor);
                seeds(x,y)=true ;
                std::cout << "Seeds: " << x << " and " << y <<std::flush;
            }
        }
    }
}
